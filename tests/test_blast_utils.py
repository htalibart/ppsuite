...
class Test_BLAST_Utils(unittest.TestCase):
    ...
    def test_get_potts_object_blast_call(self):
        cf = Potts_Object.from_files(sequence_file=SEQ_1CC8, fetch_sequences=True, sequences_fetcher='blast', database=self.db_path, feature_folder=pathlib.Path('/tmp/'+next(tempfile._get_candidate_names())))
