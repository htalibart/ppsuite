...
def get_relative_w_norms(potts_object, pdb_chain, contact_distance=8):
    w_norms = potts_object.potts_model.get_w_norms()
    rtpdb = get_real_pos_to_pdb_pos(pdb_chain, potts_object.sequence)
...
def visualize_mrf_with_contact_map(potts_object, pdb_chain, alphabet=ALPHABET, start_at_1=True, show_figure=True, **kwargs):
    relative_w_norms = get_relative_w_norms(potts_object, pdb_chain)
