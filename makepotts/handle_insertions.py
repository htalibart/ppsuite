...
def infer_insertion_penalties(input_file, free_end_gaps=True, pc_insertions_tau=0, learning_coeff_insertions=1e-3, **kwargs):
    delta_ins = count_insertions(input_file)
    L = get_length_ins_file(input_file)
    insertion_penalties = maximize_likelihood(delta_ins, L, learning_coeff_insertions=learning_coeff_insertions, pc_insertions_tau=pc_insertions_tau, **kwargs)
    if free_end_gaps:
